...
r = requests.post(
    ...
    json={"n_or_d": ident, "output-format": "json"},
)
...
t0 = time.time()
ephem_1 = query_miriade_ephemcc(ident, jd, parameters=parameters)
    ...
    if isinstance(a[0], int) or isinstance(a[0], float):
        print(
            "{}: median deviation = {:.2f}% (max = {:.2f}%)".format(
                col, np.median(np.abs(1 - a / b)) * 100, np.max(np.abs(1 - a / b)) * 100
            )
        )
    else:
...
print(
    "Median position deviation: {:.2f} arcsec (max = {:.2f} arcsec)".format(
        np.median(dev), np.max(dev)
    )
)
