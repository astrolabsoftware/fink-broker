...
if [[ -z "$FILES" ]]; then
    broadcastMessage "[Rubin] Nothing found for $NIGHT"
    exit
fi
...
for file in $FILES; do
    ...
    if [[ $isPythonProblem != "" ]]; then
        MESSAGE+="[Rubin] SPARK FAILURE: $(basename $file) $newline"
    fi

    isJsonProblem=$(grep -n JSONDecodeError "$file")
    if [[ $isJsonProblem != "" ]]; then
        MESSAGE+="[Rubin] JSON FAILURE: $(basename $file) $newline"
    fi

    isHdfsProblem=$(grep -n "Name node is in safe mode" "$file")
    if [[ $isHdfsProblem != "" ]]; then
        MESSAGE+="[Rubin] HDFS FAILURE: $(basename $file) $newline"
    fi
    ...
    if [[ $isHbaseProblem != "" ]]; then
        MESSAGE+="[Rubin] HBase FAILURE: $(basename $file) $newline"
...
if [[ -z "$MESSAGE" ]]; then
    broadcastMessage "[Rubin] All OK for $NIGHT"
else
